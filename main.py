...
def train(args):
    ...
    if args.weight is not None:
        if args.use_dp_prompt:
            checkpoint_path = os.path.join(args.weight, "{}_dp_prompt.pt".format(args.dataset))
            if not os.path.exists(checkpoint_path):
                raise FileNotFoundError(
                    "DP prompt checkpoint not found: {}. Train without --weight first.".format(checkpoint_path)
                )
            checkpoint = torch.load(checkpoint_path, map_location=device)
            clip_model.load_dp_checkpoint(checkpoint)
            capacity = clip_model.prompt_capacity(sample=False)
            logger.info("Loaded DP prompt checkpoint: {}".format(checkpoint_path))
            logger.info("Stick weights: {}".format(
                np.array2string(
                    capacity["stick_weights"].detach().cpu().numpy(), precision=4
                )
            ))
            logger.info("Prompt gates: {}".format(
                np.array2string(
                    capacity["soft_gates"].detach().cpu().numpy(), precision=4
                )
            ))
            logger.info("Active prompt atoms: {}, K*: {}".format(
                (capacity["active_indices"].detach().cpu() + 1).tolist(),
                capacity["effective_k"],
            ))
        else:
            ...
            loaded_prompt = torch.load(prompt_path, map_location=device)
            with torch.no_grad():
                clip_model.state_prompt_embedding.copy_(loaded_prompt.to(device))
            clip_model.adaptor = torch.load(adaptor_path, map_location=device)
            logger.info("Loaded AF-CLIP baseline weights: {}, {}".format(
                prompt_path, adaptor_path
            ))
    else:
        if args.use_dp_prompt:
            optimizer = torch.optim.Adam(
                clip_model.get_optimizer_parameter_groups(args.lr, args.dp_lr),
                betas=(0.5, 0.999),
            )
        else:
            optimizer = torch.optim.Adam(
                clip_model.get_trainable_parameters(), lr=args.lr, betas=(0.5, 0.999)
            )
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for items in tqdm(train_dataloader):
                ...
                gts = gts.to(device)
                if args.use_dp_prompt:
                    predict_labels, predict_masks, img_tokens, dp_aux = clip_model.detect_forward_seg(
                        imgs, args=args, return_dp_aux=True
                    )
                else:
                ...
                af_loss = focal_loss(predict_labels, labels) + args.lambda1 * (
                    focal_loss(predict_masks, gts) + l1_loss(predict_masks, gts)
                ) + args.lambda2 * patch_alignment_loss(img_tokens, labels, gts)
                if args.use_dp_prompt:
                    dp_losses = clip_model.dp_regularization_losses(dp_aux)
                    loss = af_loss + args.lambda_dp * dp_losses["stick"]
                else:
                    loss = af_loss
                optimizer.zero_grad()
                loss.backward()
                ...
                if args.use_dp_prompt:
                    af_loss_list.append(af_loss.item())
                    stick_loss_list.append(dp_losses["stick"].item())

            if args.use_dp_prompt:
                capacity = clip_model.prompt_capacity(sample=False)
                logger.info(
                    "Epoch: {}/{}, Loss: {:.6f}, AF: {:.6f}, Stick: {:.6f}, "
                    "ExpectedK: {:.3f}, K*: {}".format(
                        epoch,
                        args.epochs,
                        np.mean(total_loss),
                        np.mean(af_loss_list),
                        np.mean(stick_loss_list),
                        capacity["soft_gates"].sum().item(),
                        capacity["effective_k"],
                    )
                )
                logger.info("Stick weights: {}".format(
                ...
                logger.info("Active prompt atoms: {}".format(
                    (capacity["active_indices"].detach().cpu() + 1).tolist()
                ))
            else:
                logger.info("Epoch: {}/{}, Loss: {:.6f}".format(
                    epoch, args.epochs, np.mean(total_loss)
                ))

        if args.save_dir is not None:
            os.makedirs(args.save_dir, exist_ok=True)
            if args.use_dp_prompt:
                checkpoint_path = os.path.join(args.save_dir, "{}_dp_prompt.pt".format(args.dataset))
                torch.save(clip_model.get_dp_checkpoint(), checkpoint_path)
                logger.info("Saved DP prompt checkpoint: {}".format(checkpoint_path))
            else:
                prompt_path = os.path.join(args.save_dir, "{}_prompt.pt".format(args.dataset))
                adaptor_path = os.path.join(args.save_dir, "{}_adaptor.pt".format(args.dataset))
                torch.save(clip_model.state_prompt_embedding, prompt_path)
                torch.save(clip_model.adaptor, adaptor_path)
                logger.info("Saved AF-CLIP baseline weights: {}, {}".format(
                    prompt_path, adaptor_path
                ))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha', type=float, default=0.1, help='few-shot memory-bank fusion weight')

    parser.add_argument('--epochs', type=int, default=2, help='training epoch')
    
    parser.add_argument('--prompt_len', type=int, default=12, help='AF-CLIP baseline prompt length; V4 uses dp_kmax as its maximum length')

    parser.add_argument('--use_dp_prompt', type=int, default=1, choices=[0, 1], help='1: V4 single dynamic-capacity prompt, 0: original AF-CLIP baseline')

    parser.add_argument('--dp_kmax', type=int, default=12, help='maximum number of learnable context vectors in the single V4 prompt')

    parser.add_argument('--dp_alpha', type=float, default=1.0, help='Beta(1, alpha) concentration for stick breaking')

    parser.add_argument('--dp_gate_temperature', type=float, default=0.2, help='temperature of the differentiable prompt-capacity gates')

    parser.add_argument('--dp_gate_threshold', type=float, default=0.05, help='stick-weight threshold used to activate a prompt context vector')

    parser.add_argument('--dp_sample_sticks', type=int, default=0, choices=[0, 1], help='sample reparameterized Beta sticks during V4 training')

    parser.add_argument('--dp_lr', type=float, default=0.0001, help='learning rate for Beta stick parameters')
    ...
    parser.add_argument('--save_dir', type=str, default='./weight', help='directory used to save the trained DP checkpoint')
    ...
    parser.add_argument('--lambda_dp', type=float, default=0.00001, help='weight of the stick posterior KL loss')
